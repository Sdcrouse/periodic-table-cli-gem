...
After checking out the repo, run `bin/setup` to install dependencies. Then, run `rake spec` to run the tests (of which there are only two at this time). You can also run `bin/console` for an interactive prompt that will allow you to experiment.
